<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['base:inspectionOfficeItem:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['base:inspectionOfficeItem:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="inspectionOfficeItemList" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listInspectionOfficeItem(this.queryParams).then(response => {
        this.inspectionOfficeItemList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加检验所项目信息";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getInspectionOfficeItem(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateInspectionOfficeItem(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addInspectionOfficeItem(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除检验所项目信息编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delInspectionOfficeItem(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有检验所项目信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportInspectionOfficeItem(queryParams);
        }).then(response => {
